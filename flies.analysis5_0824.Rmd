...
setwd("/data/home/zachary.madaj/Rprojects/flies")
library(qdapRegex)
...
```{r Awake-pval,  fig.width=6,fig.height=6}

# ```{r Awake-pval,  fig.width=6,fig.height=3+length(unique(df.m$Group))/16.5}

for(i in unique(df.m$Compound)){
  ix = as.character(unique(df.m$Group[df.m$Compound %in% i]))
  ix = ix[ix != "Control"]
  tmp = unlist(sapply(ix,function(x) res.all$Comparison[grepl(x, res.all$Comparison,fixed=T)]))
  
  p_val_heatmap(format_heat(res.all[res.all$Comparison %in% tmp,]))
...
```{r Awake-FC,  fig.width=6,fig.height=6}
...
for(i in unique(df.m$Compound)){
  ix = as.character(unique(df.m$Group[df.m$Compound %in% i]))
  ix = ix[ix != "Control"]
  tmp = unlist(sapply(ix,function(x) eff.all$Comparison[grepl(x, eff.all$Comparison,fixed=T)]))
  
  eff_heatmap(format_heat(eff.all[eff.all$Comparison %in% tmp,]))
...
for(i in unique(df.m$Compound)){
  ix = as.character(unique(df.m$Group[df.m$Compound %in% i]))
  ix = ix[ix != "Control"]
  tmp = unlist(sapply(ix,function(x) sig.all$Comparison[grepl(x, sig.all$Comparison,fixed=T)]))
  
  sig_heatmap(format_heat(sig.all[sig.all$Comparison %in% tmp,]))
...
```{r Act-Awake-pval,  fig.width=6,fig.height=6}
...
```{r Act-Awake-FC,  fig.width=6,fig.height=6}
# ```{r Awake-FC,  fig.width=6,fig.height=3+length(unique(df.m$Group))/16.5}
...
```{r sleep-pval,  fig.width=6,fig.height=6}
...
```{r sleep-FC,  fig.width=6,fig.height=6}
# ```{r Awake-FC,  fig.width=6,fig.height=3+length(unique(df.m$Group))/16.5}
...
```{r total-pval,  fig.width=6,fig.height=6}
...
```{r total-FC,  fig.width=6,fig.height=6}
# ```{r Awake-FC,  fig.width=6,fig.height=3+length(unique(df.m$Group))/16.5}
...
agg.d = merge(agg.d,aggregate(value~Group,df.m[df.m$Time == "5 second data" &df.m$Measure == "average",],Hmisc::GiniMd),by="Group")

colnames(agg.d)[2:3]=c("value","gini")
...
agg.d = aggregate(value~Group,df.m[df.m$Time == "15 second data" &df.m$Measure == "average",],median)
agg.d = merge(agg.d,aggregate(value~Group,df.m[df.m$Time == "15 second data" &df.m$Measure == "average",],Hmisc::GiniMd),by="Group")
...
agg.d = aggregate(value~Group,df.m[df.m$Time == "25 second data" &df.m$Measure == "average",],median)
agg.d = merge(agg.d,aggregate(value~Group,df.m[df.m$Time == "25 second data" &df.m$Measure == "average",],Hmisc::GiniMd),by="Group")
...
df.m = df.m[tolower(df.m$Group) != "empty",]
