...
setwd("~/bbc-secondary/research/LEMH_Ellen_Fly_Shiny/44733_dom")
library(qdapRegex)
...
p_val_heatmap = function(heat.df){
  ...
  heat.m$Dataset = gsub("-","vs",heat.m$Dataset)
...
eff_heatmap = function(heat.df){
  ...
  heat.m$Effect[heat.m$Effect < -2] = -2
  ##Adding this so the FC is in the more familair direction
  
  heat.m$Effect=-1*heat.m$Effect
  heat.m$Dataset = gsub("-","vs",heat.m$Dataset)
...
<!--     fit24 = rlmer(log(value+1)~Group*Sex*Condition+(1|Dataset/ID),data=df.m[df.m$variable == "24-hour bin" &df.m$Dataset == i,]) -->
...
<!--     fitL = rlmer(log(value+1)~Group*Sex*Condition+(1|Dataset/ID),data=df.m[df.m$variable == "1st 12-hour bin" &df.m$Dataset == i,]) -->
...
<!--       fitD = rlmer(log(value+1)~Group*Sex*Condition+(1|Dataset/ID),data=df.m[df.m$variable == "2nd 12-hour bin" &df.m$Dataset == i,]) -->
...
analyze.soc = function(fxn,var){
  ...
  # for(i in unique(df.m$Dataset)){
    ...
    tmp = df.m[df.m$variable == "dist1" ,]
    tmp$Group = factor(tmp$Group,levels=c("WT",unique(tmp$Group[tmp$Group !="WT"])))
    fitd1 = rlmer(log(value+1)~Group*Sex*Condition+(1|Dataset/ID),data=tmp)
  
    res = data.frame(summary(confint(emmeans(fitd1,fxn,adjust="none"))$contrasts))
    ...
    tmp = df.m[df.m$variable == "dist2" ,]
    tmp$Group = factor(tmp$Group,levels=c("WT",unique(tmp$Group[tmp$Group !="WT"])))
    
    fitd2 = rlmer(log(value+1)~Group*Sex*Condition+(1|Dataset/ID),data=tmp)
  
    res = data.frame(confint(emmeans(fitd2,fxn,adjust="none"))$contrasts)
    ...
    colnames(eff.d2)[2] = "Distance 2"
      tmp = df.m[df.m$variable == "dist3",]
    tmp$Group = factor(tmp$Group,levels=c("WT",unique(tmp$Group[tmp$Group !="WT"])))
  
  
    fitd3 = rlmer(log(value+1)~Group*Sex*Condition+(1|Dataset/ID),data=tmp)
  
    res = data.frame(confint(emmeans(fitd3,fxn,adjust="none"))$contrasts)
    ...
    colnames(eff.d3)[2] = "Distance 3"
      ...
      tmp = df.m[df.m$variable == "av_dist" ,]
    tmp$Group = factor(tmp$Group,levels=c("WT",unique(tmp$Group[tmp$Group !="WT"])))
  
    fitad = rlmer(log(value+1)~Group*Sex*Condition+(1|Dataset/ID),data=tmp)
  
    res = data.frame(confint(emmeans(fitad,fxn,adjust="none"))$contrasts)
